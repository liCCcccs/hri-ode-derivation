...
vSym.K_BFz = 6000; vSym.K_BFx = 3000; vSym.K_BMy = 50;
...
vSym.K_CFz = 0; vSym.K_CFx = 0; vSym.K_CMy = 0;
% vSym.D_AFz = 0; vSym.D_AFx = 0; vSym.D_AMy = 0;
% vSym.D_BFz = 0; vSym.D_BFx = 0; vSym.D_BMy = 0;
vSym.D_CFz = 0; vSym.D_CFx = 0; vSym.D_CMy = 0;
vSym.K_AFzN3 = 0*5; vSym.K_AFxN3 = 0*5; vSym.K_AMyN3 = 0;
vSym.K_BFzN3 = 0*5; vSym.K_BFxN3 = 0*5; vSym.K_BMyN3 = 0;
...
vSym_est.K_AFz = 30000; vSym_est.K_AFx = 20000; vSym_est.K_AMy = 80;
vSym_est.K_BFz = 30000; vSym_est.K_BFx = 20000; vSym_est.K_BMy = 80;
...
y(1, :) = y0;
num_ft = 6;  % number of force/torque measurements in total
ft_int = zeros(length(tspan), num_ft);
for i = 2:length(tspan)
    ts = [0, 0.005];
    y0 = y(i-1, :);
    [t_tmp,y_tmp] = ode45(@(t_tmp,y_tmp) runrobot(t_tmp,y_tmp, tau, vSym), ts, y0);
    y(i, :) = y_tmp(end, :);
end

[f_intAi, f_intAj, tau_intAz, f_intBi, f_intBj, tau_intBz] = calcualte_interaction_force_torque(vSym, y(:,1), y(:,3), y(:,5), y(:,7), y(:,9), y(:,11), y(:,2), y(:,4), y(:,6), y(:,8), y(:,10), y(:,12));
ft_int(:, 1) = f_intAi;
ft_int(:, 2) = f_intAj;
ft_int(:, 3) = tau_intAz;
ft_int(:, 4) = f_intBi;
...
tspan = [0 :0.005: T];
%tspan = 0:0.05:1;
y0 = [-pi/2, 0];  %initial condition
tau = [0, 0, 0, 0];
x_est_traj = zeros(length(tspan), length(y0));
x_pred_traj = zeros(length(tspan), length(y0));
y_pred_traj = zeros(length(tspan), length(y0));
y_est_traj = zeros(length(tspan), length(y0));
x_est_traj(1, :) = y0;
x_pred_traj(1, :) = y0;
y_pred_traj(1, :) = y0(1);
y_est_traj(1, :) = y0(1);
...
for i = 2:length(tspan)
    ...
    f_int_cur = ft_int(i, :);
    r_q4 = y(i, 9);
    [t_tmp, y_tmp] = ode45(@(t_tmp, y_tmp) runKF(t_tmp, y_tmp, f_int_cur, vSym, q1, r_q4), ts, y0);

    x_pred = y_tmp(end, :);
    qr_pred = x_pred(1) + get_delta_theta(f_int_cur, vSym_est);

    x_pred_traj(i, :) = x_pred;
    y_pred_traj(i, :) = qr_pred;

    qr_mea = y(i, 11);
    x_est = x_pred + K_kf * (qr_mea - qr_pred);
    qr_est = x_est(1)+ get_delta_theta(f_int_cur, vSym);
    y_est_traj(i, :) = qr_est;
...
%% Visualise
% animate_true_obsv(vSym, vSym_est, y, x_est_traj, dt, 'n');

%% Plot trajectory
plot_true_obsv(tspan, y, x_est_traj, x_pred_traj, y_pred_traj, y_est_traj)
...
%% Verify nonlinear spring behavior
% x = 0:0.001:0.1;
% ylin = vSym.K_AFz * x;
...
%% Functions
function [d_theta] = get_delta_theta(f_int_cur, vSym)
    d_theta1 = f_int_cur(3) / vSym.K_AMy;
    d_theta2 = f_int_cur(6) / vSym.K_BMy;
    d_theta = d_theta2 - d_theta1;
end

function dydt = runrobot2(t, y, tau, vSym, ft_int)
    vVar.h_q2 = y(1); vVar.h_dq2 = y(2);
    ...
    ddq1 = 0;
    [h_ddq2, r_ddd2, r_ddd3, r_ddq4, r_ddq5] = calc_equation_of_motion2(vSym, vVar, ft_int);

    dydt = zeros(2,1);
    dydt(1) = vVar.h_dq2; dydt(2) = h_ddq2;
...
function dydt = runKF(t, y, ft_int, vSym, q1)
    ...
    dydt = zeros(2,1);
    dydt(1) = h_dq2; dydt(2) = h_ddq2;
end

function dydt = runKF_Fan2013(t, y, ft_int, vSym, q1)
    l1 = vSym.l1;  l2 = vSym.l2;  l3 = vSym.l3;  l4 = vSym.l4;
    lc1 = vSym.lc1;  la1 = vSym.la1;  lb1 = vSym.lb1;  lc2 = vSym.lc2;
    lc3 = vSym.lc3;  lc4 = vSym.lc4;  la4 = vSym.la4;  la2 = vSym.la2;
    K_AFz = vSym.K_AFz;  K_AFx = vSym.K_AFx;  K_AMy = vSym.K_AMy;
...
function [h_ddq2, r_ddd2, r_ddd3, r_ddq4, r_ddq5] = calc_equation_of_motion2(vSym, vVar, ft_int)
    % Assign values to symbols
    l1 = vSym.l1;  l2 = vSym.l2;  l3 = vSym.l3;  l4 = vSym.l4;
    lc1 = vSym.lc1;  la1 = vSym.la1;  lb1 = vSym.lb1;  lc2 = vSym.lc2;
    lc3 = vSym.lc3;  lc4 = vSym.lc4;  la4 = vSym.la4;  la2 = vSym.la2;
...
function plot_true_obsv(t, x_true, x_est, x_pred, y_pred, y_est)
    figure('Renderer', 'painters', 'Position', [300 300 800 800])
    hold on;
    grid on;
    deft_color = [[0, 0.4470, 0.7410]; [0.8500, 0.3250, 0.0980]; [0.9290, 0.6940, 0.1250]];
    subplot(3,1,1);
    hold on;
    plot(t, x_true(:, 3), 'LineWidth', 2, 'Color', deft_color(2, :), 'LineStyle','--')
    plot(t, x_pred(:, 1), 'LineWidth', 1, 'Color', deft_color(2, :))  % h_q5
    plot(t, x_est(:, 1), 'LineWidth', 2, 'Color', deft_color(2, :))  % h_q5
    %plot(t, x_true(:, 11), 'LineWidth', 2, 'Color', deft_color(1, :), 'LineStyle','--')

    legend(["True human knee angle", "Predicted human knee angle", "Filtered human knee angle"]) 
    
    subplot(3,1,2);
    hold on;
    plot(t, x_true(:, 11), 'LineWidth', 2, 'Color', deft_color(1, :), 'LineStyle','--')
    plot(t, y_pred(:, 1), 'LineWidth', 1, 'Color', deft_color(1, :))  % h_q5
    plot(t, y_est(:, 1), 'LineWidth', 2, 'Color', deft_color(1, :))  % h_q5
    
    legend(["True robot knee angle", "Predicted robot knee angle", "Filtered robot knee angle"]) 
    
    subplot(3,1,3);
    hold on;
    plot(t, x_true(:, 4), 'LineWidth', 2, 'Color', deft_color(2, :), 'LineStyle','--')
    plot(t, x_pred(:, 2), 'LineWidth', 1, 'Color', deft_color(2, :))  % h_q5
    plot(t, x_est(:, 2), 'LineWidth', 2, 'Color', deft_color(2, :))  % h_q5

    legend(["True human knee velocity", "Estimated human knee velocity", "Filtered human knee velocity"]) 
    
end

function animate_true_obsv(vSym, vSym_est, y, y_ob, dt, is_save)
%   y: true system state
%   y_ob: estimation from observer
    deft_color = [[0, 0.4470, 0.7410]; [0.8500, 0.3250, 0.0980]; [0.9290, 0.6940, 0.1250]];
    ...
    %1_est = y_ob(:, 1); h_q2_est = y_ob(:, 3); r_d2_est = y_ob(:, 5); r_d3_est = y_ob(:, 7); r_q4_est = y_ob(:, 9); r_q5_est = y_ob(:, 11);
    ...
    for i = 1:length(y)
        ...
        clf
        plot_2d_links({j1, h_j2, h_j3}, '-', deft_color(1, :));
        plot_2d_links({r_j3, r_j4, r_j5, r_j6}, '-', deft_color(2, :));
        plot_2d_links({h_cuff_A, r_j3}, ':', deft_color(3, :));
        plot_2d_links({h_cuff_B, r_cuff_B}, ':', deft_color(3, :));
        plot_2d_links({h_cuff_C, r_j6}, ':', deft_color(3, :));
        colors = [[0, 0.4470, 0.7410]; [0, 0.4470, 0.7410]; [0, 0.4470, 0.7410]; [0.8500, 0.3250, 0.0980]; [0.8500, 0.3250, 0.0980]; [0.8500, 0.3250, 0.0980]];
        plot_point({h_cuff_A, h_cuff_B, h_cuff_C, r_j3, r_cuff_B, r_j6}, colors)
    
        % plot_2d_links({j1_est, h_j2_est, h_j3_est}, '--', deft_color(1, :));
        % plot_2d_links({r_j3_est, r_j4_est, r_j5_est, r_j6_est}, '--', deft_color(2, :));
        % plot_2d_links({h_cuff_A_est, r_j3_est}, ':', deft_color(3, :));
        % plot_2d_links({h_cuff_B_est, r_cuff_B_est}, ':', deft_color(3, :));
        % plot_2d_links({h_cuff_C_est, r_j6_est}, ':', deft_color(3, :));
        % 
        view(0, 90)
        legend("Human", "Robot")
...
function plot_2d_links(points, line_style, color)
    % Plot a list of 3D points.
    %
    % Parameters:
    % points: A cell array where each cell contains a 1x3 array of 3D points.
    ...
    plot(xs, ys, 'LineWidth', 2, 'LineStyle', line_style, 'Color', color);
